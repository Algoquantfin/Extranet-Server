...
def insert_data(collection_name, csv_file):
    ...
    collection_exists = collection_name in otr_db.list_collection_names()

    # Drop the collection if it already exists
    if collection_exists:
        otr_db[collection_name].drop()
        logger.info(f"Dropped existing collection '{collection_name}'")

    # Create collection named after the date (sanitized)
    collection = otr_db[collection_name]
...
for date_folder in os.listdir(data_directory):
    ...
    if os.path.isdir(date_folder_path):
        ...
        for file in os.listdir(date_folder_path):
            ...
            # Handle OTR files
            if file.startswith('FAO_ORDER_TO_TRADE_RATIO') and file.endswith('.csv'):
                date_match = re.search(r'FAO_ORDER_TO_TRADE_RATIO_(\d{8})', file)
                if date_match:
